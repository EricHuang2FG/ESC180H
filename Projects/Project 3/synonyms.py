...
def most_similar_word(word, choices, semantic_descriptors, similarity_fn):
    most_similar_words, most_similar_similarity = [], -2
    for choice in choices:
        ...
        if similarity > most_similar_similarity:
            most_similar_words.append({choice: similarity})
            most_similar_similarity = similarity
    for pair in most_similar_words:
        word = list(pair.keys())[0]
        count = pair[word]
        if count == most_similar_similarity:
            return word # this return is guaranteed since most_similar_words is guaranteed to be non-empty
